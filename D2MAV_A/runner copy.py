...
class Runner(object):
                ...
                * geo.nm)
                ...
                if ac_id in self.agent_to_id.keys():
                    ...
                    for other_id in self.id_to_group[own_group]:
                        ...
                        ## Comment this out
                        if self.vehicle_helpers[self.current_movement[own_group]].next_route_section != self.vehicle_helpers[ac_id].next_route_section:
                            ...
                            if new_distance < self.nmac_distance and i != j:
                                # if self.current_movement[own_group] != ac_id:
                                    # print("halting vehicle: ", ac_id, "due to conflict with: ", other_id.vehicle_ID, self.halt_start)
                                nmac_flag = True
                                if self.bs.traf.distflown[i] < self.bs.traf.distflown[j]:
                                    ...
                                    #     # print("adding to halt_start")
                                    #     self.halt_start[ac_id] = self.bs.sim.simt
                                    # else:
                                    #     current_halting = self.bs.sim.simt - self.halt_start[ac_id]
                                    #     # print("checking halt time: ", current_halting, self.max_halt_time)
                                    #     if current_halting > self.max_halt_time:
                                    #         self.max_halt_time = current_halting
                                    #         # print("new max halt time: ", self.max_halt_time)
    ...
    def update_communication_groups(self):
        ...
        # id_of_interest = 'PTWYL1'
        # if id_of_interest in self.vehicle_helpers:
        #     print("Checking Route Section", self.vehicle_helpers[id_of_interest].current_route_section)
        # for i in self.id_to_group.keys():
        #     # print("Updating Comm Groups")
        #     list_1 = [element.vehicle_ID for element in self.id_to_group[i]]
        #     if 'PI30L0' in list_1:
                    ...
                    if dist <= self.communication_radius or d_2 <= self.communication_radius:
                        ...
                        if id_i in self.agent_to_id.keys():
                            ...
                            if id_j in self.agent_to_id.keys():
                                ...
                                if group_i != group_j:
                                    merge_flag_i = True
                                    # print("prepping a merge: ", id_i, id_j, self.group_to_i_r[group_i], self.group_to_i_r[group_j], [aircraft_id.vehicle_ID for aircraft_id in self.id_to_group[group_i]], [aircraft_id.vehicle_ID for aircraft_id in self.id_to_group[group_j]])
                                    if self.vehicle_helpers[id_i].current_route_section !=  self.vehicle_helpers[id_j].current_route_section and self.vehicle_helpers[id_i].current_intersection == self.vehicle_helpers[id_j].current_intersection:
                                        dist_vals_i = []
                                        dist_vals_j = []
                                        merge_flag_i = False
                                    ...
                                    if merge_flag_i:
                                        for agent in self.id_to_group[group_j]:
                                            ...
                                            # print("attempting a merge 1: ", self.group_to_i_r[group_i], self.group_to_i_r[group_j])
                                        # if self.group_to_i_r[group_i] != self.group_to_i_r[group_j]:
                                        #     if self.vehicle_helpers[id_i].current_intersection == None:
                                        #         print("Merge case 1", self.group_to_i_r[group_i], self.group_to_i_r[group_j], self.vehicle_helpers[id_i].current_route_section, self.vehicle_helpers[id_j].current_route_section, self.vehicle_helpers[id_i].current_intersection, self.vehicle_helpers[id_j].current_intersection)
                                        ...
                                        for agent in self.id_to_group[group_i]:
                                            ...
                                            # print("attempting a merge 2: ", self.group_to_i_r[group_i], self.group_to_i_r[group_j])
                                        # if self.group_to_i_r[group_j] != self.group_to_i_r[group_i]:
                                        #     if self.vehicle_helpers[id_j].current_intersection == None:
                                        #         self.group_to_i_r[group_j] = self.vehicle_helpers[id_i].current_intersection
                                        #     else:
                                        #         self.group_to_i_r[group_j] = self.vehicle_helpers[id_j].current_intersection
                                        del self.id_to_group[group_i]
                                        del self.vls[group_i]
                                    # print("end of merge: ", id_i, id_j, self.group_to_i_r[group_i], self.group_to_i_r[group_j])
                            ...
                            if id_j in self.agent_to_id.keys():
                                ...
                                if  self.vehicle_helpers[id_i].current_intersection == self.group_to_i_r[group_j] or self.vehicle_helpers[id_i].current_route_section == self.group_to_i_r[group_j]:
                                    ...
                                    if self.vehicle_helpers[id_i].current_intersection != None:
                                        ...
                                        if self.vehicle_helpers[id_i].current_route_section != None:
                                            self.group_to_i_r[self.num_groups] = self.vehicle_helpers[id_i].current_route_section
                                        else:
                                            print("Edge Case Reached 1 !!: ", id_i, self.vehicle_helpers[id_i].current_route_section)
                                            time.sleep(10)
                                    self.num_groups += 1
                            else:
                                ...
                                if self.vehicle_helpers[id_i].current_intersection != None:
                                    ...
                                    if self.vehicle_helpers[id_i].current_route_section != None:
                                        ...
                                        print("Edge Case Reached 2!!: ", id_i, self.vehicle_helpers[id_i].current_route_section)
                                        time.sleep(10)
                                    # self.group_to_i_r[self.num_groups] = self.vehicle_helpers[id_i].current_route_section
                                self.num_groups += 1
        ...
        while True:
            ...
            if term["__all__"] or self.step_counter >= self.max_steps:
                ...
                if len(self.full_travel) == 0:
                    ...
                    self.agent.data["max_travel_time"] = self.full_travel[max(self.full_travel)]
