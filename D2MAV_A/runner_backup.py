        ...
        for i in range(d.shape[0]):
            ...
            if self.vls_active:
                if id_ in self.agent_to_id.keys():
                    own_group = self.agent_to_id[id_]
                    for other_id in self.id_to_group[own_group]:
                        j = self.bs.traf.id2idx(other_id.vehicle_ID)
                        i = self.bs.traf.id2idx(id_)
                        if d[i][j] < self.nmac_distance and i != j:
                            if self.current_movement[own_group] != id_:
                                # print("halting vehicle: ", id_, "due to conflict with: ", other_id.vehicle_ID)
