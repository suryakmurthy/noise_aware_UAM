...
#
    ...
    def setclrcmd(self, 
        ...
        """ SETCLR acid
            ...
            Change the acid's clearance status."""
        
        bs.traf.active[idx] = flag

        if not bs.traf.active[idx]:
            if alt:
                self.selaltcmd(idx,alt) # go to altitude

            else:
                self.selaltcmd(idx,0) # go to ground

            if spd:
                self.selspdcmd(idx,spd) # go to speed
            else:
                self.selspdcmd(idx,0) # go to speed 0
