'''
2023/2/12 Modified from demo-version6.py to generate multiple scenario files.

Generated files stored in ~scripts/generated_scenarios/

2023/3/7 debug the time information
'''
...
class Simulator(object):
    ...
    def time_reformat(self, original_time):
        if len(str(original_time)) == 2:
            return str(original_time)
        elif len(str(original_time)) == 1:
            return "0" + str(original_time)
        else:
            print("Error: {}".format(original_time))
    ...
    def add_plane(self, plane, depart_time, route_length,route_id):
        ...
        hour_info = depart_time // 3600
        min_info = (depart_time - hour_info * 3600) // 60
        sec_info = (depart_time - hour_info * 3600 - min_info * 60)

        hour_final = self.time_reformat(hour_info)
        min_final = self.time_reformat(min_info)
        sec_final = self.time_reformat(sec_info) + '.00'

        time = "{}:{}:{}".format(hour_final, min_final, sec_final)
...
if __name__ == "__main__":
    ...
    f = open('pass_to_casestudy.json')
    data = json.load(f)
    title = 'generated_scenarios/demo-{}.scn'.format(str(data['iidx']))
    new_simulator = Simulator(title,AC_nums,Route_list,ROUTE,aclat,aclon)
